...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # Bayes Net Structure
    # initialising variables and edges
    variables = [PAC, GHOST0, GHOST1, OBS0, OBS1]
    edges = [(GHOST0, OBS0), (PAC, OBS0), (PAC, OBS1), (GHOST1, OBS1)]
    
    # initialising our positions
    all_posn = []
    for i in range(X_RANGE):
        for j in range(Y_RANGE):
            all_posn.append((i, j))
    
    # initialising out distance
    all_dist = range(0, MAX_NOISE + X_RANGE + Y_RANGE -1)
    #print("all_dist : ", all_dist)
    
    # setting values
    variableDomainsDict[PAC] = all_posn
    variableDomainsDict[GHOST0] = all_posn
    variableDomainsDict[GHOST1] = all_posn
    variableDomainsDict[OBS0] = all_dist
    variableDomainsDict[OBS1] = all_dist
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for eliminationVariable in eliminationOrder:
            currentFactorsNotToJoin, joinedFactor = joinFactorsByVariable(Factors, eliminationVariable)
            # now removing elimination variables and joining it to list of Factors
            if len(joinedFactor.unconditionedVariables()) != 1:
                removedVariableFactor = eliminate(joinedFactor, eliminationVariable)
                Factors = currentFactorsNotToJoin
                Factors.append(removedVariableFactor)
            else:
                Factors = currentFactorsNotToJoin

        # normalise all factors 
        ans_factor = normalize(joinFactors(Factors))
        # print("ans_factor: ",ans_factor,"  normalise: ",normalize(ans_factor))
